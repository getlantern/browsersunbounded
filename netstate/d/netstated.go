...
func handleExec(w http.ResponseWriter, r *http.Request) {
  ...
  case netstatecl.OpConsumerConnectionChange:
    state, workerIdx, addr := inst.Args[0], inst.Args[1], inst.Args[2]
    
    switch state {
    case "1":
      world.addVertex(vertex(inst.Tag))
      world.addEdge(vertex(inst.Tag), edge{vertex(addr), workerIdx})
    case "-1":
      world.delEdge(vertex(inst.Tag), workerIdx)
      if world.degree(vertex(addr)) == 0 {
        world.delVertex(vertex(addr))
      }
    }
  }
